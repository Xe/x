...
func (c *imageClient) Stream(ctx context.Context, opts ...grpc.CallOption) (Image_StreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Image_ServiceDesc.Streams[0], Image_Stream_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &imageStreamClient{stream}
	return x, nil
}
...
func _Image_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ImageServer).Stream(&imageStreamServer{stream})
}
