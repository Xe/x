...
`Render(oldName, newName string, newPic, old1, old2 image.Image) (image.Image, error)`
...
3. Draw text. Font: Comic Sans MS Bold. Stroke: 1px `#006488`. Names are
...
| `GET /static/...`       | Embedded static files (`treasure.gif`, CSS).                               |
...
Flags: `--bind` (default `:3000`), `--bucket`, `--presign-expiry` (default
`1h`). Credentials and endpoint from standard `AWS_*` env vars
(`AWS_ENDPOINT_URL_S3=https://t3.storage.dev`, `AWS_REGION=auto`), as in
`cmd/uploud`. No secrets in the repo.
